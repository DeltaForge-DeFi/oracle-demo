...
export function calculate(longAmount, shortAmount, leverageLong, leverageShort, data) {
  ...
  const gmxPnlYear = (shortPositionSize - gmxOpenFee - gmxSwapFee) * gmxYearNetRate / 100 - gmxCloseFee;
  ...
  const residualSupplyScenario2 = longTotalSupply * (aaveCriticalEthValue / ethPrice) - longTotalBorrow
  //количество долларов которые мы получаем с шорта после достижения критической цены по aave
  const shortProfitScenario2 = shortPositionSize - shortPositionSize * (aaveCriticalEthValue / ethPrice) - gmxCloseFee
  //итоговая сумма долларов после ликвидации лонга и заработка от шорта
  const totalUsdcScenario2 = residualSupplyScenario2 + shortProfitScenario2 + shortAmount
  ...
  const residualSupplyScenario3 = longTotalSupply * (gmxCriticalEthValue / ethPrice) - longTotalBorrow
...
for (let longPiece = 1; longPiece <= 1000; longPiece++) {
  ...
  // Перебираем плечи
  for (let leverageShort = 1; leverageShort <= 3; leverageShort++) { //плечо
    for (let leverageLong = 0; leverageLong <= 2; leverageLong++) { //количество раз взятия кредита и перекладывания его в депозит
      // Считаем стратегию
